...
def pay_menu(plan_key: str, balance: float, price: float) -> InlineKeyboardMarkup:
    rows = [
        [InlineKeyboardButton(
            text="CryptoBot",
            callback_data=f"crypto:{plan_key}",
            icon_custom_emoji_id="5260752406890711732"
        )],
        [InlineKeyboardButton(
            text="Прямой перевод",
            callback_data=f"direct:{plan_key}",
            icon_custom_emoji_id="5890848474563352982"
        )],
    ]
    if balance >= price:
        rows.append([InlineKeyboardButton(
            text=f"С баланса (${balance:g})",
            callback_data=f"balance:{plan_key}",
...
async def deliver_access(user_id: int, plan_key: str) -> None:
    ...
    try:
        ...
        await bot.send_message(
            user_id,
            f'<tg-emoji emoji-id="5870633910337015697">✅</tg-emoji> Оплата получена! '
            f'Тариф: <b>{PLANS[plan_key]["title"]}</b>\n'
...
async def cmd_start(message: Message) -> None:
    ...
    await message.answer(
        '<tg-emoji emoji-id="5870764288364252592">🙂</tg-emoji> Приветствую! '
        'Это официальный бот <b>Арсена Маркаряна</b>, '
        ...
        'Нажимай кнопку ниже '
        '<tg-emoji emoji-id="5893057118545646106">📰</tg-emoji>',
...
async def cb_home(call: CallbackQuery) -> None:
    await call.message.edit_text(
        '<tg-emoji emoji-id="5873147866364514353">🏘</tg-emoji> Главное меню:',
        reply_markup=main_menu()
    )
    await call.answer()
...
async def cb_buy(call: CallbackQuery) -> None:
    await call.message.edit_text(
        '<tg-emoji emoji-id="5904462880941545555">🪙</tg-emoji> Выберите тариф подписки:',
...
async def cb_plan(call: CallbackQuery) -> None:
    ...
    balance = await get_balance(call.from_user.id)
    await call.message.edit_text(
        f'<tg-emoji emoji-id="5886285355279193209">🏷</tg-emoji> '
        f'Тариф: <b>{plan["title"]}</b>\n'
        f'Стоимость: <b>${plan["price"]:g}</b>\n\n'
        f'<tg-emoji emoji-id="5890848474563352982">🪙</tg-emoji> Выберите способ оплаты:',
...
async def cb_crypto(call: CallbackQuery) -> None:
    ...
    await call.message.edit_text(
        f'<tg-emoji emoji-id="5260752406890711732">👾</tg-emoji> '
        f'Счёт на <b>${plan["price"]:g}</b> создан.\n'
        f'Оплатите его в CryptoBot и нажмите «Проверить оплату».',
...
async def cb_check(call: CallbackQuery) -> None:
    ...
    await call.message.edit_text(
        '<tg-emoji emoji-id="5870633910337015697">✅</tg-emoji> Оплата подтверждена!',
        reply_markup=main_menu()
    )
    await call.answer()
...
async def cb_balance(call: CallbackQuery) -> None:
    ...
    await call.message.edit_text(
        '<tg-emoji emoji-id="5870633910337015697">✅</tg-emoji> Оплачено с баланса!',
        reply_markup=main_menu()
    )
    await call.answer()
...
async def cb_promo(call: CallbackQuery, state: FSMContext) -> None:
    ...
    await call.message.edit_text(
        '<tg-emoji emoji-id="5886285355279193209">🏷</tg-emoji> Введите промокод одним сообщением:'
    )
    await call.answer()
...
async def msg_promo(message: Message, state: FSMContext) -> None:
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        if not row:
            await message.answer(
                '<tg-emoji emoji-id="5870657884844462243">❌</tg-emoji> Промокод не найден.',
                reply_markup=main_menu()
            )
            return
        if row[2]:
            await message.answer(
                '<tg-emoji emoji-id="5870657884844462243">❌</tg-emoji> Промокод уже использован.',
                reply_markup=main_menu()
            )
            return
        await db.execute(
    ...
    await message.answer(
        ...
        f'Баланс: <b>${balance:g}</b>\n\n'
        f'Теперь можно оформить подписку и оплатить её с баланса.',
        reply_markup=main_menu(),
    )
...
async def cmd_admin(message: Message) -> None:
    if not is_admin(message.from_user.id):
        return
    await message.answer(
        '<tg-emoji emoji-id="5870982283724328568">⚙️</tg-emoji> Админ-панель:',
...
async def cb_adm_promo(call: CallbackQuery, state: FSMContext) -> None:
    ...
    await call.message.edit_text(
        '<tg-emoji emoji-id="5870676941614354370">🖋</tg-emoji> '
...
async def cb_adm_stats(call: CallbackQuery) -> None:
    ...
    await call.message.edit_text(
        ...
        f'<tg-emoji emoji-id="5870772616305839506">👥</tg-emoji> Пользователей: <b>{users}</b>\n'
        f'<tg-emoji emoji-id="5870633910337015697">✅</tg-emoji> Активных подписок: <b>{active}</b>\n'
        f'<tg-emoji emoji-id="5886285355279193209">🏷</tg-emoji> Неиспользованных промокодов: <b>{promos}</b>\n'
        f'<tg-emoji emoji-id="5879814368572478751">🏧</tg-emoji> Успешных оплат: <b>{paid}</b>',
        reply_markup=admin_menu(),
    )
    await call.answer()
...
async def cb_approve(call: CallbackQuery) -> None:
    ...
    await deliver_access(row[0], row[1])
    await call.message.edit_text(
        f'<tg-emoji emoji-id="5870633910337015697">✅</tg-emoji> Заявка #{payment_id} подтверждена.'
    )
    await call.answer()
...
async def cb_reject(call: CallbackQuery) -> None:
    ...
    try:
        await bot.send_message(
            ...
            'Ваша оплата отклонена администратором.'
        )
    except Exception:
        pass
    await call.message.edit_text(
        f'<tg-emoji emoji-id="5870657884844462243">❌</tg-emoji> Заявка #{payment_id} отклонена.'
    )
    await call.answer()
            ...
            async with aiosqlite.connect(DB_PATH) as db:
                ...
                for user_id, plan_key, expires_at in to_warn:
                    if PLANS.get(plan_key, {}).get("seconds", 0) <= WARN_BEFORE:
                        ...
                        continue
                    until = time.strftime("%d.%m.%Y %H:%M", time.localtime(expires_at))
                    try:
                        await bot.send_message(
                            user_id,
                            f'<tg-emoji emoji-id="5983150113483134607">⏰</tg-emoji> '
                            f'Ваша подписка заканчивается <b>{until}</b> '
                            f'(через ~2 дня). Продлите доступ, чтобы остаться в канале.',
                            reply_markup=main_menu(),
                        )
                    except Exception as e:
                ...
                for (user_id,) in expired:
                    ...
                    try:
                        await bot.send_message(
                            user_id,
                            '<tg-emoji emoji-id="6037249452824072506">🔒</tg-emoji> '
